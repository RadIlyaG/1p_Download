...
class SoapWS(MacReg):
   ...
   <soapenv:Body>
      <mod:get_Data_4_Dallas soapenv:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/">
        ...
        if '502 Proxy Error' in response.content.decode():
            return False, f"Failed to get Pages for ID:{id} TraceID:{trace_id}: 502 Proxy Error"
...
if __name__ == '__main__':
    ...
    if True:
        ...
        res, p_dict = sws.get_pages('IO3001960310', "50190576", 0)
        if res:
            print(p_dict)
        else:
            print(p_dict)

        res, p_dict = sws.get_pages('DF1002709336', "20648029", 0)
        if res:
            print(p_dict)
        else:
            print(p_dict)
    ...
    if False:
        ...
        #  IO3001208539 ETX-203AX_TWC/N/GE30/2SFP/2UTP2SFP
        res, dicti = ws.retrieve_mkt_number('ETX-203AX_TWC/N/GE30/2SFP/2UTP2SFP/DROP')
        # print(type(res), type(dicti))
        print(res, dicti['MKT_PDN'])

        res, dicti = ws.retrieve_mkt_number('ETX-203AX_TWC/N/GE30/2SFP/2UTP2SFP')
        # print(type(res), type(dicti))
        print(res, dicti['MKT_PDN'])

        # print(type(res), type(dicti))
        res, dicti = ws.retrieve_oi4barcode('IO3001208539')
        # print(type(res), type(dicti))
        print('OI4Barcode', res, dicti['item'])
        res, dicti = ws.retrieve_oi4barcode('DF1002708294')
        # print(type(res), type(dicti))
        print('OI4Barcode', res, dicti['item'])
